...
import com.drawing.BalloonTextStrokeFactory
...
class BalloonTextStrokeFactoryTest {
    ...
    @Test
    fun `drawn multiline text preserves blank lines between paragraphs`() {
        val bounds = Rectangle(10, 10, 180, 120)
        val twoLines = BalloonTextStrokeFactory.buildTextStrokes(
            text = "A\nA",
            bounds = bounds,
            color = Color.BLUE,
            style = BalloonTextStyle.SOLID
        ).flatMap { it.points }
        val threeLines = BalloonTextStrokeFactory.buildTextStrokes(
            text = "A\n\nA",
            bounds = bounds,
            color = Color.BLUE,
            style = BalloonTextStyle.SOLID
        ...
        val twoLineSpan = twoLines.maxOf { it.dy } - twoLines.minOf { it.dy }
        val threeLineSpan = threeLines.maxOf { it.dy } - threeLines.minOf { it.dy }

        assertTrue(twoLines.isNotEmpty(), "Control text should create strokes")
        assertTrue(threeLines.isNotEmpty(), "Blank-line text should create strokes")
        assertTrue(
            threeLineSpan >= twoLineSpan + 8,
            "A preserved blank line should increase the vertical span"
        )
    }

    @Test
    fun `outline long words wrap instead of overflowing the box`() {
        val bounds = Rectangle(10, 10, 26, 140)
        val strokes = BalloonTextStrokeFactory.buildTextStrokes(
        ...
        assertTrue(strokes.isNotEmpty(), "Long outline text should still produce strokes")
        assertTrue(points.isNotEmpty(), "Long outline text should still have visible points")
        assertTrue(
            points.maxOf { it.dx } <= bounds.x + bounds.width + 4,
            "Wrapped long words should stay within the requested width"
        )
        assertTrue(
            points.minOf { it.dx } >= bounds.x - 4,
            "Wrapped long words should stay near the left edge of the requested width"
        )
    }
}
