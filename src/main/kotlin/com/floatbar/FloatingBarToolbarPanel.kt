...
internal data class FloatingBarToolbarControls(
...
internal data class FloatingBarToolbarView(
    val rootPanel: JPanel,
    val headerPanel: JPanel,
    val controls: FloatingBarToolbarControls
)

internal class FloatingBarToolbarPanel(
    ...
    private val buttonFactory = FloatingBarButtonFactory(restoreToolbarStyles)

    fun create(): FloatingBarToolbarView {
    ...
    private fun createHistoryRow(): JPanel {
        return JPanel(GridLayout(1, 2, 6, 0)).apply {
            ...
            maximumSize = Dimension(FloatingBarMetrics.BAR_WIDTH, FloatingBarMetrics.BUTTON_HEIGHT)
            preferredSize = Dimension(FloatingBarMetrics.BAR_WIDTH, FloatingBarMetrics.BUTTON_HEIGHT)
        }
    }
    ...
    private fun createControls(): FloatingBarToolbarControls {
        val recentColorButtons = createRecentColorButtons()
        return FloatingBarToolbarControls(
    ...
    private fun createToolStatusLabel(): JLabel {
        return JLabel("Tool: Draw", SwingConstants.CENTER).apply {
            preferredSize = Dimension(FloatingBarMetrics.BAR_WIDTH, FloatingBarMetrics.STATUS_HEIGHT)
            maximumSize = Dimension(FloatingBarMetrics.BAR_WIDTH, FloatingBarMetrics.STATUS_HEIGHT)
            alignmentX = Component.CENTER_ALIGNMENT
    ...
    private fun createColorStatusLabel(): JLabel {
        return JLabel("Color: #000000", SwingConstants.CENTER).apply {
            preferredSize = Dimension(FloatingBarMetrics.BAR_WIDTH, FloatingBarMetrics.COLOR_STATUS_HEIGHT)
            maximumSize = Dimension(FloatingBarMetrics.BAR_WIDTH, FloatingBarMetrics.COLOR_STATUS_HEIGHT)
            alignmentX = Component.CENTER_ALIGNMENT
    ...
    private fun addToolbarSections(
        buttonColumn: JPanel,
        historyRow: JPanel,
        controls: FloatingBarToolbarControls
    ) {
    ...
    private fun createSectionLabel(text: String): JLabel {
        return JLabel(text.uppercase(), SwingConstants.LEFT).apply {
            preferredSize = Dimension(FloatingBarMetrics.BAR_WIDTH, 16)
            maximumSize = Dimension(FloatingBarMetrics.BAR_WIDTH, 16)
            alignmentX = Component.CENTER_ALIGNMENT
    ...
    private fun createRecentSectionHeader(): JLabel {
        return JLabel("RECENT COLORS", SwingConstants.LEFT).apply {
            preferredSize = Dimension(FloatingBarMetrics.BAR_WIDTH + 16, 16)
            maximumSize = Dimension(FloatingBarMetrics.BAR_WIDTH + 16, 16)
    ...
    private fun createSectionSeparator(): JPanel {
        return JPanel().apply {
            maximumSize = Dimension(FloatingBarMetrics.BAR_WIDTH, 8)
            preferredSize = Dimension(FloatingBarMetrics.BAR_WIDTH, 8)
            minimumSize = Dimension(FloatingBarMetrics.BAR_WIDTH, 8)
            isOpaque = false
