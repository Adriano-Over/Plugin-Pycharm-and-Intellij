...
internal object FloatingBarStateBinder {
    ...
    fun applyPassThroughButton(button: JButton, enabled: Boolean) {
        ...
        button.toolTipText = if (enabled) {
            "Let the editor receive clicks and drags while drawings stay visible. FloatBar will remember this choice"
        } else {
            "Block the editor with the drawing overlay again. FloatBar will remember this choice"
        }
        applyToggleStyle(button, enabled)
    }
    ...
    fun shapeButtonText(activeTool: FloatBarToolMode, selectedShapeName: String): String {
        return when (activeTool) {
            FloatBarToolMode.SHAPES -> selectedShapeName
            else -> "Shapes"
        }
    }

    fun applyToolButton(button: JButton, active: Boolean, tool: FloatBarToolMode) {
        if (active) {
    ...
    fun applyShapeToolButton(button: JButton, active: Boolean) {
        applyToolButton(button, active, FloatBarToolMode.SHAPES)
    }

    fun applyToolStatusLabel(label: JLabel, activeTool: FloatBarToolMode, selectedShapeName: String) {
    ...
    fun toolStatusText(activeTool: FloatBarToolMode, selectedShapeName: String): String {
        return when (activeTool) {
            FloatBarToolMode.SELECT -> "Tool: Select"
            FloatBarToolMode.DRAW -> "Tool: Draw"
            FloatBarToolMode.ERASE -> "Tool: Erase"
            FloatBarToolMode.FILL -> "Tool: Fill"
            FloatBarToolMode.SHAPES -> "Tool: $selectedShapeName"
        }
    }
    ...
    private fun toolAccentColors(tool: FloatBarToolMode): Pair<Color, Color> {
        return when (tool) {
            FloatBarToolMode.SELECT -> Color(48, 96, 118) to Color(130, 215, 240)
