...
internal object BalloonTextStrokeFactory {
    ...
    fun buildTextStrokes(
        ...
        if (cleanText.isEmpty() || bounds.width <= 4 || bounds.height <= 4) {
            return emptyList()
        }

        if (style == BalloonTextStyle.SOLID) {
            return buildDrawnTextStrokes(cleanText, bounds, color)
        }

        val fontRenderContext = FontRenderContext(null, true, true)
        val font = chooseFont(cleanText, bounds, fontRenderContext)
        val lines = wrapText(cleanText, font, fontRenderContext, bounds.width)
        if (lines.isEmpty()) {
            return emptyList()
        }
    ...
    private fun wrapText(
        ...
        for (paragraph in text.lines()) {
            ...
            for (word in words) {
                ...
                if (outlineTextWidthPx(candidate, font, fontRenderContext) <= maxWidth || currentLine.isEmpty()) {
    ...
    private fun wrapDrawnText(text: String, maxLineWidthPx: Double, fontSize: Double): List<String> {
        ...
        for (paragraph in text.lines()) {
            ...
            for (word in words) {
                val candidate = if (currentLine.isEmpty()) word else "$currentLine $word"
                if (drawnTextWidthPx(candidate, fontSize) <= maxLineWidthPx || currentLine.isEmpty()) {
                    currentLine = candidate
                } else {
                    lines += currentLine
                    currentLine = word
