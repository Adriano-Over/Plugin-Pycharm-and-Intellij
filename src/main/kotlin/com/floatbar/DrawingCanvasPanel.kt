...
class DrawingCanvasPanel(
    ...
    private var foldListenerDisposable = Disposer.newDisposable("FloatBarFoldListener")
    ...
    init {
        FloatBarDiagnosticLog.configure(project)
        FloatBarDiagnosticLog.info("PANEL", "DrawingCanvasPanel created")
        isOpaque = false
        ...
        val inputController = DrawingInputController(
            ...
            clampPoint = { point, tool ->
                coordinateMapper.clampPointToDrawableArea(
                    point = point,
                    allowCodeArea = tool == FloatBarToolMode.ERASE || tool == FloatBarToolMode.SELECT,
                    rejectCodeArea = tool == FloatBarToolMode.FILL
                )
            },
    ...
    fun bindEditor(editor: Editor) {
        FloatBarDiagnosticLog.info("PANEL", "bindEditor beforePersist currentFile=${currentFile?.path} strokes=${currentStrokes().size}")
        ...
        FloatBarDiagnosticLog.info("PANEL", "bindEditor newFile=${currentFile?.path} documentLines=${editor.document.lineCount}")
        currentStroke = null
        shapePreview = null
        ...
        FloatBarDiagnosticLog.info("PANEL", "bindEditor loadedStrokes=${currentStrokes().size}")
    ...
    fun unbindEditor() {
        FloatBarDiagnosticLog.info("PANEL", "unbindEditor file=${currentFile?.path} strokes=${currentStrokes().size}")
        documentSync.persistCurrentStrokes()
        documentSync.unbindDocumentListener()
        unbindFoldListener()
        editor = null
    ...
    fun setSelectMode() {
        setTool(FloatBarToolMode.SELECT)
    }
    ...
    private fun setTool(tool: FloatBarToolMode, clearPreview: Boolean = true) {
        ...
        if (tool != FloatBarToolMode.SELECT) {
    ...
    private fun updateToolCursor() {
        cursor = if (interactionPassThroughEnabled) {
            ...
            when (currentTool) {
                FloatBarToolMode.SELECT -> Cursor.getPredefinedCursor(Cursor.MOVE_CURSOR)
    ...
    private fun unbindFoldListener() {
        ...
        foldListenerDisposable = Disposer.newDisposable("FloatBarFoldListener")
    }
