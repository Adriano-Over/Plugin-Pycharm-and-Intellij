...
object FloatBarDebugLog {
    private val log = Logger.getInstance("FloatBar")
    private val lastLogByKey = ConcurrentHashMap<String, Long>()

    private const val DEFAULT_THROTTLE_MS = 500L

    fun info(message: String) {
        log.info("[FloatBar] $message")
    }

    fun warn(message: String, throwable: Throwable? = null) {
        if (throwable == null) {
            log.warn("[FloatBar] $message")
        } else {
            log.warn("[FloatBar] $message", throwable)
        }
    }
